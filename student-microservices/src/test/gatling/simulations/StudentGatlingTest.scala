...
class StudentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Student entity")
        ...
        .repeat(2) {
            ...
            .get("/microservicesstudent/api/students")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new student")
            .post("/microservicesstudent/api/students")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "surname":"SAMPLE_TEXT", "faculty":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_student_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created student")
                .get("/microservicesstudent${new_student_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created student")
            .delete("/microservicesstudent${new_student_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
